...
export function calculateAllMethods(
  ...
  const availableLots = getAvailableLots(lots, disposalDate);

  return {
    FIFO: calculateFIFO(availableLots, quantity).totalCostBasis,
    LIFO: calculateLIFO(availableLots, quantity).totalCostBasis,
    HIFO: calculateHIFO(availableLots, quantity).totalCostBasis,
  };
}
 ...
 */
export function calculateCostBasis(request: DisposalRequest, lots: CryptoLot[]): DisposalResult {
  const availableLots = getAvailableLots(lots, request.disposalDate);

  switch (request.method) {
    case 'FIFO':
      return calculateFIFO(availableLots, request.quantity);
    case 'LIFO':
      return calculateLIFO(availableLots, request.quantity);
    case 'HIFO':
      return calculateHIFO(availableLots, request.quantity);
    case 'SpecificID':
      ...
      return calculateSpecificID(availableLots, request.quantity, request.specificLotIds);
    default:
 ...
 */
function calculateLIFO(lots: CryptoLot[], quantity: string): DisposalResult {
 ...
 */
function calculateHIFO(lots: CryptoLot[], quantity: string): DisposalResult {
 ...
 */
function allocateFromLots(lots: CryptoLot[], quantity: string): DisposalResult {
 ...
 */
function getAvailableLots(lots: CryptoLot[], date?: string): CryptoLot[] {
  if (!date) {
 ...
 */
export function updateLotsAfterDisposal(
  lots: CryptoLot[],
 ...
 */
export function calculateWeightedAverage(lots: CryptoLot[]): string {
 ...
 */
export function validateLots(lots: CryptoLot[]): { isValid: boolean; errors: string[] } {
  const errors: string[] = [];
