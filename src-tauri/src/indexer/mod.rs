...
use crate::core::{Transaction, ChainConfig};
use anyhow::Result;
...
impl PolkadotIndexer {
    pub fn new() -> Self {
        ...
        configs.insert("polkadot".to_string(), ChainConfig {
        ...
        configs.insert("kusama".to_string(), ChainConfig {
        ...
        configs.insert("moonbeam".to_string(), ChainConfig {
    ...
    pub async fn connect(&mut self, chain: &str) -> Result<()> {
        if let Some(config) = self.configs.get(chain) {
            let url = config.ws_endpoint.as_ref()
                .unwrap_or(&config.rpc_endpoint);
